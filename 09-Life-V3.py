...
class Field():
    ...
    def wrigt_to_file(self):
        fl_dict={'point':[]}
        for a in range(1, self.field_h):
            for b in range(1, self.field_w):
                curren_dic={'x':b,'y':a,'status':self.play_field[a][b].status}
                fl_dict['point'].append(curren_dic)
        with open("feeld.g",'w') as f:
            json.dump(fl_dict, f)

    def read_from_file(self):
        with open("feeld.g",'r') as f:
            js=json.load(f)
        for p in js['point']:
            self.play_field[p['y']][p['x']].set_status(p['status'])
...
# game_field.wrigt_to_file()
game_field.read_from_file()
